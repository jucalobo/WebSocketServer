...
var comparacionWS = function comparacionWS(obj1, obj2) {
...
var servidorIO = function servidorIO() {
  wss.on('connection', function (ws, req) {
    ...
    var URL = req.url == '/' ? 'Default' : req.url;
    aConexiones.push({
      url: URL,
      ws: ws
    ...
    ws.on('message', function (data) {
      console.log("WS send ".concat(URL, ":").concat(data));
      aConexiones.forEach(function (cliente) {
        if (cliente.url === URL) {
    ...
    ws.on('close', function () {
      ...
      aConexiones.splice(aConexiones.findIndex(function (cliente) {
        return comparacionWS(cliente.ws, ws);
      }), 1); // console.log('cerrado puerto', aConexiones);
